...
export class RemoveInterviewUseCase {
  constructor(
  ...
  async execute(id: string, tenantId: string, userId: string): Promise<string> {
    ...
    const co_user = user?.companyUsers?.find((usr) => {
      return (
        usr.companyId == company.id && (usr.role == 'ADMIN' || usr.role == 'HR')
      );
    });
    ...
    await this.interviewRepo.RemoveInterview(interview.id);
